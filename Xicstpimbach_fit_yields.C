...
void Xicstpimbach_fit_yields()
{
  ...
  //  RooAddPdf sigXicstpim("sigXicstpim", "voigt1+voigt2+voigt3+threshold", RooArgList(voigt1, voigt2, threshold), RooArgList(voigt1frac, voigt2frac,nbkg));
 ...
 //---------------------------------------Plotting on TCanvas --------------------------------------//
  ...
  RooFitResult* res = sigXicstpim.fitTo(Xicstpim_data, Save(), Optimize(false), Extended());
